        ...
        for conn in (edges1 & edges2):
            ...
            try:
                ...
                if symbolic_positive:
                    ...
                    volume_sum = sum(m.subset.num_elements() for m in memlets)
                    volume_union = union_subset.num_elements()
                    if volume_sum.free_symbols == volume_union.free_symbols:
                        # diff cannot be evaluated, so it still must contain a symbol 
                        # in this case diff cannot be < 0
                '''
                print("**************")
                print("TypeError here")
                print(union_subset.num_elements(), "<?", sum(m.subset.num_elements() for m in memlets))
                # NOTE: in this case union_subset.volume presumably has more 
                # symbolic characters than sum.volume, so volume can't be smaller
                pass
