...
class ArithmeticCounter:
    _indent = 0
    _iprint = lambda *args: print('  ' * _indent, *args)

    # Do not use O(x) or Order(x) in sympy, it's not working as intended
    _bigo = sympy.Function('bigo')


    # TODO: add more library node estimator functions here 
    def count_matmul(self, node: MatMul, symbols: Dict[str, Any],
                        state: dace.SDFGState) -> int:
            '''
            FLOP estimaton function for matrix multiplication library nodes.
            '''
            ...
            if len(C_memlet.data.subset) == 3:
                result *= self.symeval(C_memlet.data.subset.size()[0], symbols)
            # M*N
            result *= self.symeval(C_memlet.data.subset.size()[-2], symbols)
            result *= self.symeval(C_memlet.data.subset.size()[-1], symbols)
            # K
            result *= self.symeval(A_memlet.data.subset.size()[-1], symbols)
            return result
    ...
    def count_arithmetic_ops(self, sdfg: dace.SDFG,
                            symbols: Dict[str, Any] = None) -> int:
        result = 0
        symbols = symbols or {}
        for state in sdfg.nodes():
            result += self.count_arithmetic_ops_state(state, symbols) * state.executions
        return result


    def symeval(self, val: Any, symbols: Dict[str, Any]):
    ...
    def evalsyms(self, base: Dict[str, Any], new: Dict[str, Any]):
        result = {}
        for k, v, in new.items():
            result[k] = self.symeval(v, base)
        return result


    def count_arithmetic_ops_state(self, state: dace.SDFGState,
                                symbols: Dict[str, Any] = None) -> int:
        symbols = symbols or {}
        ...
        def traverse(scope: Scope) -> int:
            ...
            for node in sdict[scope.entry]:
                ...
                if isinstance(node, dace.nodes.NestedSDFG):
                    ...
                    nested_syms.update(self.evalsyms(symbols, node.symbol_mapping))

                    #iprint('SUBSDFG', node.sdfg.name, nested_syms)
                    self._indent += 1
                    node_result += self.count_arithmetic_ops(node.sdfg, nested_syms)
                    self._indent -= 1
                    ...
                    if node.code.language == dace.Language.CPP:
                        ...
                        node_result += self.count_arithmetic_ops_code(
                            node.code.code)
                elif isinstance(node, dace.libraries.standard.nodes.reduce.Reduce):
                    node_result += state.in_edges(node)[
                        0].data.subset.num_elements() * self.count_arithmetic_ops_code(
                            node.wcr)
                elif isinstance(node, dace.nodes.LibraryNode):
                    node_result += self.LIBNODES_TO_ARITHMETICS[type(node)](self, node,
                                                                    symbols,
                                                                    state)
                ...
                if isinstance(node, (dace.nodes.CodeNode, dace.nodes.AccessNode)):
                    for oedge in state.out_edges(node):
                        if oedge.data.wcr is not None:
                            node_result += self.count_arithmetic_ops_code(
                                oedge.data.wcr)
                if node_result != 0 and self._indent <= 1:
                    if isinstance(node, dace.nodes.NestedSDFG) and self._indent == 0:
                        self._iprint('*',
                            type(node).__name__, node.sdfg.name, ': `',
                            node_result, '`')
                    elif isinstance(node, dace.nodes.Tasklet) and self._indent == 1:
                        self._iprint('*',
            ...
            for child in scope.children:
                if self._indent == 0:
                    self._iprint('Scope', child.entry)
                self._indent += 1
    ...
    def count_arithmetic_ops_subgraph(self, subgraph: SubgraphView, state: dace.SDFGState,
                                symbols: Dict[str, Any] = None) -> int:
    ...
    def count_arithmetic_ops_code(self, 
            code_or_block: Union[List[ast.AST], ast.AST, str]) -> int:
        ctr = ArithmeticASTCounter()
...
class ArithmeticASTCounter(ast.NodeVisitor):
    def __init__(self):
        self.count = 0
    ...
    def visit_Call(self, node: ast.Call):
        ...
        if fname not in self.PYFUNC_TO_ARITHMETICS:
            print('WARNING: Unrecognized python function "%s"' % fname)
            return self.generic_visit(node)
        self.count += self.PYFUNC_TO_ARITHMETICS[fname]
        return self.generic_visit(node)
...
if __name__ == '__main__':
    ...
    ops = counter.count_arithmetic_ops(sdfg)
