...
class RegisterScore(MemletScore):
    ...
    def estimate_spill(self, sdfg, graph, subgraph):
        ...
        reg_count_available = 0
        threads = Config.get('compiler', 'default_block_size')
        threads = functools.reduce(lambda a, b: a*b, [int(e) for e in threads.split(',')])
        self._registers_per_thread = self.register_per_block/(max(2048, ))
    ...
    def score(self, subgraph: SubgraphView):
        ...
        spill_traffic = self.estimate_spill(sdfg_copy, graph_copy, subgraph_copy)
