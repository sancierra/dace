...
def list_top(subgraph_scores, n=10, list_all=False):
    ...
    for i, (subgraph, _) in enumerate(subgraph_scores):
        for j, (other_subgraph, _) in enumerate(subgraph_scores):
            if i != j and all(s in other_subgraph for s in subgraph) and all(
                    s in subgraph for s in other_subgraph):
                dupes += 1
...
def get_sdfg(program_name: str, gpu: bool):
    '''
    ...
    if 'hdiff' not in program_name:
        expand_reduce(sdfg,
                      graph,
                      reduce_implementation='pure'
                      if not gpu else 'CUDA (block allreduce)')
