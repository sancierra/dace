...
def consolidate_edges_scope(
    ...
    '''
      for e in inner_edges(scope_node):
        connector = e.src_conn if isinstance(scope_node, nd.EntryNode else e.dst_conn))
        edges_by_connector[connector].append(e)
        if e.data.data not in data_to_conn:
            data_to_conn[e.data.data] = connector
        elif data_to_conn[e.data.data] != connector:  # Need to consolidate
            connectors_to_remove.add(connector)
    print("CONN REMOVE", list(connectors_to_remove))
